        ...
        return <div className={classes.wrapper}>
            <div>
                {pendingBills.sort((a, b) => b.Timestamp - a.Timestamp).map((bill) => {
                    return <div key={bill.Id} className={classes.listItem}>
                        <div className={classes.billCard}>
                            <div className={classes.billContent}>
                                <div className={classes.billAmount}>{bill.Amount}</div>
                                <div className={classes.billTitle}>{bill.Title}</div>
                                {bill.Account && <div className={classes.billInfo}>Account Number: {bill.Account}</div>}
                                <div className={classes.billInfo}>
                                    Received <Moment unix date={bill.Timestamp} fromNow />
                                </div>
                                {bill.Description && <div className={classes.billDescription}>{bill.Description}</div>}
                            </div>
                            <div className={classes.billActions}>
                                <IconButton className={`${classes.actionButton} ${classes.errorText}`} onClick={() => onDenyBill(bill)}>
                                    <FontAwesomeIcon icon={['fas', 'xmark']} />
                                </IconButton>
                                <IconButton className={`${classes.actionButton} ${classes.successText}`} onClick={() => openAcceptBilling(bill)}>
